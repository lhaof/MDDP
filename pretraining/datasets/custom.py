...
class CustomAlignedDataset(Dataset):
    def __init__(self, dataset_config, stage='train'):
        ...
        if stage == 'test' or stage=='val':
            image_paths_ori,label_ori = get_image_paths_from_dir(os.path.join(dataset_config.dataset_path, 'valB'))
            image_paths_cond,label_cond = get_image_paths_from_dir(os.path.join(dataset_config.dataset_path, 'valA'))
            self.label_ori = label_ori
        else:
            image_paths_ori,label_ori = get_image_paths_from_dir(os.path.join(dataset_config.dataset_path, 'trainB'))
            image_paths_cond,label_cond = get_image_paths_from_dir(os.path.join(dataset_config.dataset_path, 'trainA'))
            label_ori.extend(label_cond)
            self.label_ori = label_ori
        self.flip = dataset_config.flip if stage == 'train' else False
        self.to_normal = dataset_config.to_normal
        ...
        print(len(self.imgs_ori),len(self.label_ori))
    def __len__(self):
        return len(self.imgs_ori)

    def __getitem__(self, i):
        return self.imgs_ori[i], self.imgs_cond[i],self.label_ori[i]
