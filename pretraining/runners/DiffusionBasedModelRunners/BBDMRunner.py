...
class BBDMRunner(DiffusionBaseRunner):
    ...
    def loss_fn(self, net, batch, epoch, step, opt_idx=0, stage='train', write=True):
        (x, x_name), (x_cond, x_cond_name),label = batch
        x = x.to(self.config.training.device[0])
        x_cond = x_cond.to(self.config.training.device[0])

        recloss,contrastive_loss, additional_info = net(x, x_cond,label)
        loss = recloss + contrastive_loss
        if write:
            self.writer.add_scalar(f'loss/{stage}', loss, step)
            if additional_info.__contains__('contrastive_loss'):
                self.writer.add_scalar(f'contrastive_loss/{stage}', additional_info['contrastive_loss'], step)
