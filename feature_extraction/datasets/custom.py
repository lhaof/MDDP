...
class CustomAlignedDataset(Dataset):
    def __init__(self, dataset_config, stage='train'):
        ...
        if stage == 'test' or stage=='val':
            image_paths_ori = get_image_paths_from_dir(os.path.join(dataset_config.dataset_path, 'valB'))
            image_paths_cond = get_image_paths_from_dir(os.path.join(dataset_config.dataset_path, 'valA'))
        else:
            image_paths_ori = get_image_paths_from_dir(os.path.join(dataset_config.dataset_path, 'trainB'))
            image_paths_cond = get_image_paths_from_dir(os.path.join(dataset_config.dataset_path, 'trainA'))
        self.flip = dataset_config.flip if stage == 'train' else False
        self.to_normal = dataset_config.to_normal
