...
class BrownianBridgeModel(nn.Module):
    ...
    def p_sample(self, x_t, y, context, i, clip_denoised=False):
        ...
        if self.steps[i] == 0:
            ...
            x_t, _ = self.q_sample(x_t, y, t) ###if not add noise
            iter_feature,objective_recon = self.denoise_fn(x_t, timesteps=t, context=context)
            #objective_recon = self.denoise_fn(x_t, timesteps=t, context=context)
            x0_recon = self.predict_x0_from_objective(x_t, y, t, objective_recon=objective_recon)
            #print(x0_recon.shape)
            x0_recon = self.gap(x0_recon).squeeze(-1).squeeze(-1)
            iter_feature.append(x0_recon)
    ...
    def p_sample_loop(self, y, context=None, clip_denoised=True, sample_mid_step=False):
        ...
        if sample_mid_step:
            ...
            #for i in tqdm(range(len(self.steps)), desc=f'sampling loop time step', total=len(self.steps)):
            #for i in tqdm(range(len(self.steps)), desc=f'sampling loop time step', total=len(self.steps)):
            #for i in tqdm([100], desc=f'sampling loop time step', total=1):
            img = self.p_sample(x_t=img, y=y, context=context, i=20, clip_denoised=clip_denoised)
            #x0_recon = self.gap(x0_recon).squeeze(-1).squeeze(-1)
            return img
